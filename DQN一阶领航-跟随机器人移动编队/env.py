...
class Env:
    ...
    def step(self, actions):
        ...
        # leader 移动速度 2*pi*r/1000 = 0.12
        x = 2 * np.cos(self.step_ctr / 100 * np.pi)
        y = 2 * np.sin(self.step_ctr / 100 * np.pi)
        self.pos[-1] = np.array([x, y])

        # 二阶动力学，必须要用到速度信息，否则智能体会困惑同样的地方奖励不同
        # for i in range(self._num_followers):
        #     self.acc[i] = self.action[actions[i]]*0.1
