...
class Env:
    ...
    def reset(self):
        ...
        state = np.dot(self.L, current_pos).reshape(5, 2)
        self.step_ctr = 0
        return state
    ...
    def calculate_reward(self, current_pos, next_pos):
        ...
        #     # 假设别的机器人都没有动，只有当前机器人动了
        #     im_pos = current_pos.copy()
        #     im_pos[i:2 * (i + 1)] = next_pos[i:2 * (i + 1)]
        #     im_current_relative_distance = np.dot(self.L, im_pos)
        #     # 上一时刻与目标队形的差距
        #     previous_distance = np.linalg.norm(previous_relative_distance[2 * i:2 * (i + 1)] -
        #                                        target_relative_distance[2 * i:2 * (i + 1)])
        #     current_distance = np.linalg.norm(im_current_relative_distance[2 * i:2 * (i + 1)] -
        #                                       target_relative_distance[2 * i:2 * (i + 1)])
        ...
        for i in range(self._num_agent):
            ...
            if abs(current_distance) <= 0.2:
                ...
                t = True
            # 超出边界
            # if np.any(next_pos[2 * i:2 * (i + 1)] >= 10) or np.any(next_pos[2 * i:2 * (i + 1)] <= -10):
            #     r = -1
            #     t = True
            reward.append(r)
            terminated.append(t)
    ...
    def step(self, actions):
        ...
        # 二阶动力学
        # for i in range(self.num_agent):
        #     self.acc[i] = self.action[actions[i]]
        ...
        # 计算相对偏移
        current_pos = self.pos.flatten()
        next_state = np.dot(self.L, current_pos).reshape(5, 2)
        return next_state, reward, truncated, terminated

    def render(self):
        plt.clf()
        bound = np.ones(101)
        axis = np.arange(-10, 10 + 0.2, 20 / 100)
        ...
        # formation = np.append(formation, formation[0]).reshape(5, 2)
        # plt.plot(formation[:, 0], formation[:, 1], label='formation shape')
        # plt.plot(bound * 10, axis, color='blue')
        # plt.plot(bound * -10, axis, color='blue')
        # plt.plot(axis, bound * 10, color='blue')
        # plt.plot(axis, bound * -10, color='blue')
